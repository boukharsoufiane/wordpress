 ...
 */
function pure_blog_get_google_fonts_url() {
	$fonts_url = '';

	$fonts = apply_filters( 'pure_blog_google_fonts', array() );
 ...
 */
function pure_blog_get_social_share( $id, $sites = array() ) {
	...
	if ( ! empty( $links ) ) {
		...
		foreach ( $links as $link ) {
			$output .= '<li><a href="' . esc_url( $link['url'] ) . '" class="' . esc_attr( $link['id'] ) . '" target="_blank">' . pure_blog_get_theme_svg( $link['id'], 'social' ) . '</a></li>';
		}
 ...
 */
function pure_blog_the_theme_svg( $svg_name, $group = 'ui', $color = '' ) {
	echo pure_blog_get_theme_svg( $svg_name, $group, $color ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in pure_blog_get_theme_svg().
}
 ...
 * @param string $color    Color code.
 */
function pure_blog_get_theme_svg( $svg_name, $group = 'ui', $color = '' ) {
	// Make sure that only our allowed tags and attributes are included.
	$svg = wp_kses(
		Pure_Blog_SVG_Icons::get_svg( $svg_name, $group, $color ),
		array(
			'svg'     => array(
				'class'       => true,
				'xmlns'       => true,
 ...
 */
function pure_blog_apply_theme_shortcode( $text ) {
	if ( empty( $text ) ) {
		return $text;
	}
	...
	$replace = array(
		date_i18n( esc_html_x( 'Y', 'year date format', 'pure-blog' ) ),
 ...
 * @since 1.0.0
 */
function pure_blog_menu_fallback() {
	echo '<ul id="menu-main-menu" class="menu">';

	echo '<li class="menu-item"><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'pure-blog' ) . '</a></li>';
 ...
 */
function pure_blog_get_user_details( $user_id ) {
	$output = array();
